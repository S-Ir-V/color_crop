...
def mylog_prob(self, x):
    # 假设 self.all_log_prob 的形状是 (n_batch, n_actions)
    selected_p = F.select_item(self.all_log_prob, x)
    return selected_p
##########################

class A3C_InnerState(agent.AttributeSavingMixin, agent.AsyncAgent):
...
##############################
        ...
        total_loss = F.mean(pi_loss + v_loss)
